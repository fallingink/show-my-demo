<template>
  <header>
    <nav :class="[$style['h-screen-8'],'tw-dy-breadcrumbs tw-flex tw-justify-center tw-text-gray-800 tw-bg-indigo-300']">
      <ul>
        <li>
        ...
        <li>
          <label class="tw-dy-swap tw-swap-rotate">
            ...
            <svg
              :class="['tw-fill-current tw-w-10 tw-h-10',themesOffType]"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            >
              <path
...
function initThemes():void{
  ...
  if (val) {
    if (val === "winter") {
      ...
      themesOffType.value = "tw-dy-swap-off";
    } else {
      themesOnType.value = "tw-dy-swap-off";
      themesOffType.value = "tw-dy-swap-on";
    }
  }
}
